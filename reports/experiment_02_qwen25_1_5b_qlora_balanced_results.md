...
Experiment 01 used Qwen2.5-0.5B with QLoRA on the imbalanced training set. 
The fine-tuned model became biased toward `no_trace`.
...
Total balanced training examples:

```text
1606 Validation and test sets were unchanged.
...
Keeping the balanced training data.
Testing Qwen2.5-3B or Qwen2.5-7B on the university server.
Improving trace recall further.
Computing loss only on the answer label rather than the full prompt.
Keeping the same prompt format for training and evaluation.
