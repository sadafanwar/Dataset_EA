...
PROGRESS="results/${EXP}/size_200_fold_progress_v5.csv"
STEP_LOG="results/${EXP}/size_200_step_log_v5.csv"
...
log_step () {
  ...
  echo "${train_size},${fold},${step},${status},${start_iso},${end_iso},${duration},${artifact},${notes}" >> "$STEP_LOG"
}
...
for FOLD_NUM in 01 02 03 04 05 06 07 08 09 10; do
  ...
  TRAIN_FILE="data/qwen35_oversampling_10fold_cv/inner_splits/${FOLD}/train_size_200_undersampled.jsonl"
  CONFIG="src/integration/training_scripts/ft/config_qwen35_9b_train_size_undersampling_v2_size_200_${FOLD}.yaml"
  ...
  TRAIN_LOG="${RUN_LOG_DIR}/size_200_${FOLD}_train.log"
  EVAL_LOG="${RUN_LOG_DIR}/size_200_${FOLD}_eval.log"

  run_step "$TRAIN_SIZE" "$FOLD" "subset_created" "$TRAIN_FILE" "Create balanced undersampled train subset with 100 trace and 100 no_trace" \
    python scripts/create_train_size_undersampled_subset.py \
      --fold ${FOLD_NUM#0} \
      --train_size 200

  run_step "$TRAIN_SIZE" "$FOLD" "config_created" "$CONFIG" "Create v2 QLoRA config pointing to size_200 train file and v2 output directory" \
...
echo "All requested size_200 folds 01-10 completed."
