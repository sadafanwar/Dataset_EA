...
def main():
    ...
    if template_path is None:
        raise FileNotFoundError("Could not find existing inner-validation config template.")

    train_file = f"data/qwen35_oversampling_10fold_cv/inner_splits/{fold_name}/inner_train.jsonl"
    validation_file = f"data/qwen35_oversampling_10fold_cv/inner_splits/{fold_name}/validation.jsonl"
    output_dir = f"outputs/qwen35_9b_qlora_unbalanced_10fold_cv_inner_val/{fold_name}"

    if not Path(train_file).exists():
        raise FileNotFoundError(f"Missing inner train file: {train_file}")
    ...
    note = (
        "# Qwen3.5-9B QLoRA 10-fold CV with INNER VALIDATION and NO CLASS BALANCING\n"
        "# Training uses original imbalanced inner_train.jsonl.\n"
    ...
    out_path = Path(
        ...
        f"config_qwen35_9b_10fold_cv_inner_val_unbalanced_{fold_name}.yaml"
    )
