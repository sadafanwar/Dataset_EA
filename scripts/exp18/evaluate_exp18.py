#!/usr/bin/env python3
"""Evaluate one trained EXP-18 fold on its unseen test split."""
...
from scripts.exp18.exp18_data import (  # noqa: E402
    PRIMARY_TEMPLATE_ID,
    build_prompt as build_exp18_prompt,
    load_jsonl,
    validate_raw_record,
)

from scripts.exp18.run_exp18_fold import (  # noqa: E402
    REGISTRY_FILE,
    REGISTRY_HEADERS,
    atomic_write_csv,
    read_csv_rows,
...
def build_prompt(
    ...
    """
    Build the EXP-18 primary-comparison prompt.

    Primary evaluation always uses template_01, which is
    identical to the canonical EXP-17 instruction.
    """

    return build_exp18_prompt(
        record,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate one trained EXP-18 fold "
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        print()
        print("=" * 70)
        print("EXP-18 EVALUATOR DRY RUN PASSED")
        print(
    ...
    try:
        ...
        for index, raw_record in enumerate(
            ...
            prediction = {
                ...
                "evaluation_protocol": (
                    "primary_canonical_template"
                ),
                "test_template": PRIMARY_TEMPLATE_ID,
                "fold": fold_name,
                "test_index": index,
        ...
        print()
        print("=" * 70)
        print("EXP-18 FOLD EVALUATION COMPLETED")
        print(f"Fold      : {fold_name}")
        print(
            "Accuracy  : "
