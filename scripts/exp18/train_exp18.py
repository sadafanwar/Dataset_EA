...
from scripts.exp18.exp18_data import (  # noqa: E402
    ...
    build_primary_inference_prompts,
    build_training_texts,
    build_validation_texts,
    load_jsonl,
    validate_raw_record,
)

from scripts.exp18.exp18_training_dataset import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train one EXP-18 outer cross-validation fold."
        )
    )
...
def prepare_fold_data(
    ...
    validation_texts = build_validation_texts(
        raw_validation,
        seed=template_seed,
    )

    test_prompts = build_primary_inference_prompts(
        raw_test
    )
    ...
    return {
        ...
        "train_template_counts": (
            train_template_counts
        ),
        "validation_template_counts": (
            validation_template_counts
        ),
        "test_template_counts": (
            test_template_counts
        ),
    }
...
def corrected_training_loop(
    ...
    """
    Execute EXP-18 training with correct remainder handling.
...
def main() -> int:
    ...
    print(
        "Train      : "
        f"{fold_data['train_template_counts']}"
    )

    print(
        "Validation : "
        f"{fold_data['validation_template_counts']}"
    )

    print(
        "Test       : "
        f"{fold_data['test_template_counts']}"
    )
    ...
    run_metadata = {
        ...
        "template_assignment_method": (
            "sha256_pair_id_seed_modulo"
        ),
        "template_seed": seed,
        "primary_test_template": "template_01",
        "train_template_counts": (
            fold_data["train_template_counts"]
        ),
        "validation_template_counts": (
            fold_data[
                "validation_template_counts"
        ...
        "test_template_counts": (
            fold_data["test_template_counts"]
        ),
    ...
    if args.dry_run:
        ...
        print("=" * 70)
        print(
            "EXP-18 TRAINING DRY RUN PASSED"
        )
