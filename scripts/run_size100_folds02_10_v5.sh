...
PROGRESS="results/${EXP}/size_100_fold_progress_v5.csv"
STEP_LOG="results/${EXP}/size_100_step_log_v5.csv"
...
for FOLD_NUM in 02 03 04 05 06 07 08 09 10; do
  ...
  TRAIN_FILE="data/qwen35_oversampling_10fold_cv/inner_splits/${FOLD}/train_size_100_undersampled.jsonl"
  CONFIG="src/integration/training_scripts/ft/config_qwen35_9b_train_size_undersampling_v2_size_100_${FOLD}.yaml"
  ...
  TRAIN_LOG="${RUN_LOG_DIR}/size_100_${FOLD}_train.log"
  EVAL_LOG="${RUN_LOG_DIR}/size_100_${FOLD}_eval.log"

  run_step "$TRAIN_SIZE" "$FOLD" "subset_created" "$TRAIN_FILE" "Create balanced undersampled train subset with 50 trace and 50 no_trace" \
    python scripts/create_train_size_undersampled_subset.py \
      --fold ${FOLD_NUM#0} \
      --train_size 100

  run_step "$TRAIN_SIZE" "$FOLD" "config_created" "$CONFIG" "Create v2 QLoRA config pointing to size_100 train file and v2 output directory" \
...
echo "All requested size_100 folds 02-10 completed."
