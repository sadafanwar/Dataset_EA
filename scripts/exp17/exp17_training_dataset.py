"""EXP-17 causal-LM dataset with prompt-token loss masking."""
...
class SupervisedCausalDataset(Dataset):
    """
    Tokenize EXP-17 records while masking prompt tokens.

    Labels use:
    - -100 for instruction, HLR, LLR, and "Answer:"
...
class SupervisedCausalCollator:
    """Pad inputs and preserve -100 label masking."""
...
def create_train_dataloader(
    ...
    """Build the EXP-17 target-masked DataLoader."""
...
def create_validation_dataloader(
    ...
    """
    Build a deterministic target-masked validation DataLoader.

    The complete validation fold is used. Prompt tokens and
    padding tokens retain the -100 loss mask.
    """

    if tokenizer.pad_token_id is None:
        if tokenizer.eos_token_id is None:
            raise ValueError(
                "Tokenizer defines neither PAD nor EOS token."
            )
    ...
    return DataLoader(
        ...
        shuffle=False,
        collate_fn=SupervisedCausalCollator(
            tokenizer
        ),
        num_workers=int(
            data_config.get(
