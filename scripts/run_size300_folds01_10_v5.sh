...
PROGRESS="results/${EXP}/size_300_fold_progress_v5.csv"
STEP_LOG="results/${EXP}/size_300_step_log_v5.csv"
...
for FOLD_NUM in 01 02 03 04 05 06 07 08 09 10; do
  ...
  TRAIN_FILE="data/qwen35_oversampling_10fold_cv/inner_splits/${FOLD}/train_size_300_undersampled.jsonl"
  CONFIG="src/integration/training_scripts/ft/config_qwen35_9b_train_size_undersampling_v2_size_300_${FOLD}.yaml"
  ...
  TRAIN_LOG="${RUN_LOG_DIR}/size_300_${FOLD}_train.log"
  EVAL_LOG="${RUN_LOG_DIR}/size_300_${FOLD}_eval.log"

  run_step "$TRAIN_SIZE" "$FOLD" "subset_created" "$TRAIN_FILE" "Create balanced undersampled train subset with 150 trace and 150 no_trace" \
    python scripts/create_train_size_undersampled_subset.py \
      --fold ${FOLD_NUM#0} \
      --train_size 300

  run_step "$TRAIN_SIZE" "$FOLD" "config_created" "$CONFIG" "Create v2 QLoRA config pointing to size_300 train file and v2 output directory" \
...
echo "All requested size_300 folds 01-10 completed."
