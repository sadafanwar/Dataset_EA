...
def build_traceability_prompt(instruction: str, input_obj: Dict[str, Any]) -> str:
...
def create_answer_only_data_collator(tokenizer):
...
def load_training_data(
    ...
    answer_only_loss = training_config.get("answer_only_loss", False)
   ...
   # Detect dataset format
    ...
    def tokenize_function(examples):
        ...
        if has_messages:
            ...
            for instruction, input_obj, output in zip(
                ...
                prompt = build_traceability_prompt(instruction, input_obj)
            ...
            texts = [t for t in examples["content"] if t and t.strip()]
        ...
        if answer_only_loss and has_instruction_format:
    ...
    if answer_only_loss and has_instruction_format:
        logger.info("Using answer-only loss: prompt tokens will be masked")
        data_collator = create_answer_only_data_collator(tokenizer)
    else:
