#!/usr/bin/env python3
"""Fold 01 preflight for Gemma QLoRA with the original imbalanced distribution."""
...
DEFAULT_CONFIG = REPO_ROOT / (
    "gemma_traceability/configs/config_gemma_2_9b_qlora_unbalanced_fold_01.yaml"
)
...
def setup_logging(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("gemma_2_9b_qlora_unbalanced_preflight")
...
def main() -> int:
    ...
    try:
        ...
        if any(token in Path(data["train_file"]).name for token in forbidden):
            raise ValueError("Balanced training file selected for gemma_2_9b_qlora_unbalanced")
        ...
        logger.exception("Gemma QLoRA unbalanced preflight failed")
    finally:
