#!/usr/bin/env python3
"""Gemma QLoRA 10-fold traceability experiment with frozen EXP-06 oversampling."""
...
DEFAULT_CONFIG = REPO_ROOT / (
    "gemma_traceability/configs/config_gemma_2_9b_qlora_oversampling_10fold.yaml"
)
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if config["data"]["balancing"] != "random_oversampling_with_replacement":
        raise ValueError("This experiment must use random oversampling with replacement")
    ...
    if oversampling != {
        ...
        raise ValueError("Oversampling protocol differs from corrected historical EXP-06")
    if config["training"]["dataloader_shuffle"]:
        raise ValueError("Historical EXP-06 used DataLoader shuffle=False")
    if config["prompt"]["chat_template"]:
        raise ValueError("Historical EXP-06 did not use a chat template")
    if config["prompt"]["loss_scope"] != "full_prompt_and_label":
        raise ValueError("Historical EXP-06 trained on full prompt-and-label loss")
    if config["model"]["class_name"] != "Gemma2ForCausalLM":
    ...
    if training["max_steps"] is not None:
        raise ValueError("Historical EXP-06 used two complete epochs, not max_steps")
    if training["optimizer"] != "AdamW" or training["scheduler"] != "linear_with_warmup":
        raise ValueError("Optimizer or scheduler differs from historical EXP-06")
    if training["partial_accumulation_policy"] != (
        "discard_incomplete_accumulation_at_each_epoch_historical_exp06"
    ):
        raise ValueError("Partial gradient accumulation policy differs from EXP-06")
    if not config["validation"]["compute_perplexity"]:
        raise ValueError("Historical EXP-06 monitored validation perplexity")
    if config["evaluation"]["labels_order"] != LABELS:
        raise ValueError("Evaluation label order must match historical EXP-06")
    ...
    if config["evaluation"]["invalid_prediction_policy"] != (
        ...
        raise ValueError("Invalid-prediction policy differs from historical EXP-06")
    if config["checkpointing"]["best_model_criterion"] != (
        "lowest_epoch_training_loss_historical_exp06"
    ):
        raise ValueError("Best-model selection differs from historical EXP-06")
    return config


def setup_logging(log_path: Path) -> logging.Logger:
    log_path.parent.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("gemma_2_9b_qlora_oversampling_10fold")
...
def split_paths(config: dict[str, Any], fold: int) -> dict[str, Path]:
    ...
    gemma_test_root = resolve_repo_path(config["data"]["gemma_test_root"])
    return {
        "source_train": qwen_root / "inner_splits" / name / "inner_train.jsonl",
        "train": qwen_root / "inner_splits" / name / "train_oversampled.jsonl",
        "validation": qwen_root / "inner_splits" / name / "validation.jsonl",
        "qwen_test": qwen_root / "folds" / name / "test.jsonl",
        "test": gemma_test_root / name / "test.jsonl",
    }
...
def audit_fold(config: dict[str, Any], manifest: dict[str, Any], fold: int) -> dict[str, Any]:
    ...
    test, test_audit = audit_dataset(paths["test"], expected["test"], "gemma_test")
    if test_audit["sha256"] != qwen_test_audit["sha256"]:
        raise ValueError(f"{name} Gemma test fold is not identical to the frozen Qwen test fold")
    expected_seed = config["data"]["oversampling"]["seed_base"] + fold
    if expected["oversampling_seed"] != expected_seed:
        raise ValueError(f"{name} oversampling seed differs from seed_base + fold")
    if reproduce_historical_oversampling(source_train, expected_seed) != train:
        raise ValueError(
            f"{name} frozen train file does not exactly reproduce historical EXP-06 oversampling"
        )
    train_ids = record_ids(train, allow_duplicates=True)
    ...
    return {
        ...
        "oversampling_seed": expected_seed,
...
def require_passed_preflight(config: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = {
        ...
        "oversampling_fold_seed": config["data"]["oversampling"]["seed_base"] + 1,
...
def load_bf16_base_for_evaluation(config: dict[str, Any]) -> Any:
    if config["evaluation"]["base_model_precision"] != "bfloat16":
        raise ValueError("Historical EXP-06 final evaluation used a bfloat16 base model")
    model_config = config["model"]
    model = Gemma2ForCausalLM.from_pretrained(
        model_config["id"],
        revision=model_config["revision"],
...
def train_one_fold(
    ...
    if len(dataloader) != training["expected_train_microbatches_per_fold"]:
        raise RuntimeError(
            f"{name} expected {training['expected_train_microbatches_per_fold']} "
    ...
    if total_optimizer_steps != training["scheduler_total_steps_per_fold"]:
        raise RuntimeError("Scheduler step count differs from historical EXP-06")
    ...
    for epoch in range(training["num_epochs"]):
        ...
        if expected_steps != training["expected_optimizer_steps_per_epoch"]:
            raise RuntimeError("Optimizer steps per epoch differ from historical EXP-06")
    ...
    if global_step != training["expected_completed_optimizer_steps_per_fold"]:
        raise RuntimeError("Completed optimizer steps differ from historical EXP-06")
...
def create_summary(config: dict[str, Any]) -> None:
    ...
    json_path = result_root / "gemma_2_9b_qlora_oversampling_10fold_summary.json"
    csv_path = result_root / "gemma_2_9b_qlora_oversampling_10fold_summary.csv"
    ...
    for fold in selected:
        ...
        try:
            ...
            track_fold(
                ...
                metrics,
                "Frozen EXP-06 train_oversampled.jsonl; validation/test unchanged; frozen test used only after training.",
            )
