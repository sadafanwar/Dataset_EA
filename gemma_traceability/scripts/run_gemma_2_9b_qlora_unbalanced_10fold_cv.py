#!/usr/bin/env python3
"""Ministral QLoRA 10-fold traceability experiment on original imbalanced splits."""
...
DEFAULT_CONFIG = REPO_ROOT / (
    "gemma_traceability/configs/config_gemma_2_9b_qlora_unbalanced_10fold.yaml"
)
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if config["data"]["balancing"] != "none_original_imbalanced_inner_train":
        raise ValueError("This experiment must use the original imbalanced distribution")
    ...
    if config["training"]["dataloader_shuffle"]:
        raise ValueError("Historical EXP-08 used DataLoader shuffle=False")
    if config["prompt"]["chat_template"]:
        raise ValueError("Historical EXP-08 did not use a chat template")
    if config["prompt"]["loss_scope"] != "full_prompt_and_label":
        raise ValueError("Historical EXP-08 trained on full prompt-and-label loss")
    ...
    if training["max_steps"] is not None:
        raise ValueError("Historical EXP-08 used two complete epochs, not max_steps")
    if training["optimizer"] != "AdamW" or training["scheduler"] != "linear_with_warmup":
        raise ValueError("Optimizer or scheduler differs from historical EXP-08")
    if not config["validation"]["compute_perplexity"]:
        raise ValueError("Historical EXP-08 monitored validation perplexity")
    if config["evaluation"]["labels_order"] != LABELS:
        raise ValueError("Evaluation label order must match historical EXP-08")
    ...
    if config["evaluation"]["invalid_prediction_policy"] != (
        ...
        raise ValueError("Invalid-prediction policy differs from historical EXP-08")
    if config["checkpointing"]["best_model_criterion"] != (
        "lowest_epoch_training_loss_historical_exp08"
    ):
        raise ValueError("Best-model selection differs from historical EXP-08")
    return config


def setup_logging(log_path: Path) -> logging.Logger:
    log_path.parent.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("gemma_2_9b_qlora_unbalanced_10fold")
...
def split_paths(config: dict[str, Any], fold: int) -> dict[str, Path]:
    ...
    return {
        "train": qwen_root / "inner_splits" / name / "inner_train.jsonl",
...
def audit_dataset(path: Path, expected: dict[str, Any], split: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for key in ("records", "trace", "no_trace", "sha256"):
...
def load_bf16_base_for_evaluation(config: dict[str, Any]) -> Any:
    if config["evaluation"]["base_model_precision"] != "bfloat16":
        raise ValueError("Historical EXP-08 final evaluation used a bfloat16 base model")
    model_config = config["model"]
    model = Gemma2ForCausalLM.from_pretrained(
        model_config["id"],
        revision=model_config["revision"],
...
def create_summary(config: dict[str, Any]) -> None:
    ...
    json_path = result_root / "gemma_2_9b_qlora_unbalanced_10fold_summary.json"
    csv_path = result_root / "gemma_2_9b_qlora_unbalanced_10fold_summary.csv"
    ...
    for fold in selected:
        ...
        try:
            ...
            track_fold(
                ...
                metrics,
                "Original imbalanced inner_train.jsonl; no over/undersampling; frozen test used only after training.",
            )
