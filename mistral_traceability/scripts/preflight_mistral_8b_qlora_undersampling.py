#!/usr/bin/env python3
"""Fold 01 preflight for Ministral QLoRA with frozen EXP-07 undersampling."""
...
DEFAULT_CONFIG = REPO_ROOT / (
    "mistral_traceability/configs/config_mistral_8b_qlora_undersampling_fold_01.yaml"
)
...
def setup_logging(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("mistral_8b_qlora_undersampling_preflight")
...
def main() -> int:
    ...
    try:
        ...
        if data["balancing"] != "random_undersampling_without_replacement":
            raise ValueError("The undersampling experiment must sample without replacement")
        if Path(data["train_file"]).name != "train_undersampled.jsonl":
            raise ValueError("The undersampling train_file must be frozen train_undersampled.jsonl")
        if Path(data["source_inner_train_file"]).name != "inner_train.jsonl":
            raise ValueError("The undersampling source must be frozen inner_train.jsonl")
        if "oversampled" in Path(data["train_file"]).name:
            raise ValueError("Oversampling data is forbidden in this experiment")
        ...
        if test_audit["sha256"] != qwen_test_audit["sha256"]:
            raise ValueError("Mistral and Qwen Fold 01 test files are not byte-identical")
        reproduced_train = reproduce_historical_undersampling(
            source_records, data["undersampling"]["fold_seed"]
        )
        if reproduced_train != train_records:
            raise ValueError(
                "Frozen Fold 01 train file does not exactly reproduce historical EXP-07 undersampling"
            )
        ...
        report["hyperparameters"] = {
            ...
            "undersampling_fold_seed": data["undersampling"]["fold_seed"],
        ...
        logger.exception("Ministral QLoRA undersampling preflight failed")
    finally:
