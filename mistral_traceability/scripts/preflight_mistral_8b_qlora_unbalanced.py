#!/usr/bin/env python3
"""Fold 01 preflight for Ministral QLoRA with the original imbalanced distribution."""
...
DEFAULT_CONFIG = REPO_ROOT / (
    "mistral_traceability/configs/config_mistral_8b_qlora_unbalanced_fold_01.yaml"
)
...
def setup_logging(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("mistral_8b_qlora_unbalanced_preflight")
...
def main() -> int:
    ...
    try:
        ...
        if data["balancing"] != "none_original_imbalanced_inner_train":
            raise ValueError("The unbalanced experiment must use the original imbalanced inner-train distribution")
        if Path(data["train_file"]).name != "inner_train.jsonl":
            raise ValueError("The unbalanced experiment train_file must be the frozen inner_train.jsonl")
        forbidden = ("oversampled", "undersampled")
        if any(token in Path(data["train_file"]).name for token in forbidden):
            raise ValueError("Balanced training file selected for mistral_8b_qlora_unbalanced")
        ...
        logger.exception("Ministral QLoRA unbalanced preflight failed")
    finally:
